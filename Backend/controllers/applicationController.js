  ...
  try {
    ...
    if (req.file) {
      try {
        ...
        const filename = `resume-${req.user.id}-${Date.now()}-${req.file.originalname}`;
...
const downloadResume = async (req, res) => {
  try {
    ...
    try {
      // Priority: Use GridFS if fileId exists
      if (resumeFileId) {
        console.log('Downloading from GridFS...');
        fileResponse = await getFromGridFS(resumeFileId, 'resumes');
        const fileMetadata = await getFileMetadata(resumeFileId, 'resumes');
        downloadMethod = 'gridfs';

        console.log('GridFS download successful, size:', fileResponse.length, 'bytes');

        // Set headers for download
        res.setHeader('Content-Type', fileMetadata.contentType || 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(fileName)}"`);
        res.setHeader('Content-Length', fileMetadata.length || fileResponse.length);
        res.setHeader('Cache-Control', 'no-cache');

        // Send the file
        res.send(fileResponse);

        console.log(`✅ Resume downloaded successfully via GridFS: ${fileName} (${fileResponse.length} bytes)`);
        console.log('=== DOWNLOAD RESUME SUCCESS ===');
        return;
      }

      // Fallback: Use Cloudinary URL (legacy support)
