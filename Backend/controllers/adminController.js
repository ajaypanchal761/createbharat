...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const [
      ...
      WebDevelopmentLead.countDocuments(),
      ...
      MentorBooking.aggregate([
        { $match: { paymentStatus: 'completed', status: 'completed', amount: { $exists: true } } },
        { $group: { _id: null, total: { $sum: '$amount' } } }
      ]),
      LegalSubmission.aggregate([
        { $match: { paymentStatus: 'completed', status: 'completed', paymentAmount: { $exists: true } } },
    ...
    // Calculate payment counts and totals by type (matching payments page structure)
    // Get all completed payments for counts
    const certificatePaymentsCount = await UserTrainingProgress.countDocuments({
      certificatePaymentStatus: 'completed',
      certificateAmount: { $exists: true }
    });
    const mentorPaymentsCount = await MentorBooking.countDocuments({
      paymentStatus: 'completed',
      amount: { $exists: true },
      status: 'completed'
    });
    const legalPaymentsCount = await LegalSubmission.countDocuments({
      paymentStatus: 'completed',
    ...
    // Mentor payments by day (matching payments page: only completed bookings with completed payment)
    const mentorRevenueTrend = await MentorBooking.aggregate([
      {
        $match: {
          paymentStatus: 'completed',
          status: 'completed', // Only show completed bookings (matching payments page)
          amount: { $exists: true },
    ...
    // Legal payments by day (matching payments page: only completed submissions with completed payment)
    const legalRevenueTrend = await LegalSubmission.aggregate([
      {
        $match: {
          paymentStatus: 'completed',
          status: 'completed', // Only show completed submissions (matching payments page)
          paymentAmount: { $exists: true },
          paidAt: { $gte: thirtyDaysAgo }
        }
      },
    ...
    legalRevenueTrend.forEach(item => {
      const date = item._id;
      revenueMap.set(date, (revenueMap.get(date) || 0) + item.revenue);
    });
    ...
    res.status(200).json({
      ...
      data: {
        ...
        payments: {
          ...
          mentorRevenue,
          legalRevenue,
          certificateCount: certificatePaymentsCount,
          mentorCount: mentorPaymentsCount,
          legalCount: legalPaymentsCount
        },
        mentorBookings: {
          total: totalBookings,
