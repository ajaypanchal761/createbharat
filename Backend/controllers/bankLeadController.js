...
const createLead = async (req, res) => {
  try {
    ...
    const normalizedAccountType = typeof accountType === 'string' ? accountType.toLowerCase() : accountType;
    const normalizedBusinessType = typeof businessType === 'string' ? businessType.toLowerCase() : businessType;
    const normalizedHasPanCard = typeof hasPanCard === 'string' ? hasPanCard.toLowerCase() : hasPanCard;
    const normalizedHasGst = typeof hasGst === 'string' ? hasGst.toLowerCase() : hasGst;
    const normalizedHasUdyamMsmeCertificate = typeof hasUdyamMsmeCertificate === 'string' ? hasUdyamMsmeCertificate.toLowerCase() : hasUdyamMsmeCertificate;
    const normalizedAadhaarNumber = typeof aadhaarNumber === 'string' ? aadhaarNumber.replace(/\s+/g, '') : aadhaarNumber;
    const normalizedPanNumber = normalizedHasPanCard === 'yes' && typeof panNumber === 'string'
      ? panNumber.toUpperCase().replace(/\s+/g, '')
      : '';
    const normalizedGstNumber = normalizedHasGst === 'yes' && typeof gstNumber === 'string'
      ? gstNumber.toUpperCase().replace(/\s+/g, '')
      : '';
    const normalizedUdyamMsmeNumber = normalizedHasUdyamMsmeCertificate === 'yes' && typeof udyamMsmeNumber === 'string'
      ? udyamMsmeNumber.trim()
      : '';
    ...
    const lead = await BankLead.create({
      ...
      hasPanCard: normalizedHasPanCard,
      hasGst: normalizedHasGst,
      hasUdyamMsmeCertificate: normalizedHasUdyamMsmeCertificate,
      panNumber: normalizedPanNumber || undefined,
      gstNumber: normalizedGstNumber || undefined,
      udyamMsmeNumber: normalizedUdyamMsmeNumber || undefined,
      fullName,
      email,
      phone,
      dateOfBirth,
    ...
    // Determine notification email recipient
    const notificationEmail =
      process.env.BANK_LEAD_NOTIFICATION_EMAIL ||
      process.env.CREATEBHARAT_OFFICIAL_EMAIL ||
      process.env.ADMIN_NOTIFICATION_EMAIL ||
      process.env.SMTP_USER ||
      'createbharatofficial@gmail.com';
            ...
            <div class="section">
              ...
              <div class="info-box">
                <div style="margin-bottom: 10px;">
                  ...
                  <span class="info-value">${normalizedAccountType ? normalizedAccountType.charAt(0).toUpperCase() + normalizedAccountType.slice(1) + ' Account' : 'N/A'}</span>
                </div>
                <div style="margin-bottom: 10px;">
                ...
                <div style="margin-bottom: 10px;">
                  ...
                  <span class="info-value">${normalizedBusinessType ? normalizedBusinessType.charAt(0).toUpperCase() + normalizedBusinessType.slice(1) : 'N/A'}</span>
                </div>
              </div>
            </div>
            ...
            <div class="section">
              ...
              <div class="info-box">
                <div>
                  ...
                  <span class="info-value">${normalizedAadhaarNumber || 'N/A'}</span>
                </div>
                <div style="margin-top: 10px;">
                  <span class="info-label">PAN Card:</span>
                  <span class="info-value">${normalizedHasPanCard ? normalizedHasPanCard.toUpperCase() : 'N/A'}${normalizedPanNumber ? ` (${normalizedPanNumber})` : ''}</span>
                </div>
                <div style="margin-top: 10px;">
                  <span class="info-label">GST Registration:</span>
                  <span class="info-value">${normalizedHasGst ? normalizedHasGst.toUpperCase() : 'N/A'}${normalizedGstNumber ? ` (${normalizedGstNumber})` : ''}</span>
                </div>
                <div style="margin-top: 10px;">
                  <span class="info-label">Udyam/MSME Certificate:</span>
                  <span class="info-value">${normalizedHasUdyamMsmeCertificate ? normalizedHasUdyamMsmeCertificate.toUpperCase() : 'N/A'}${normalizedUdyamMsmeNumber ? ` (${normalizedUdyamMsmeNumber})` : ''}</span>
                </div>
              </div>
            </div>
    ...
    const emailResult = await sendEmail(notificationEmail, emailSubject, emailHtml);
