...
// @desc    Get legal service by ID (for CA - includes inactive)
// @route   GET /api/ca/legal-services/:id
// @access  Private/CA
const getCAServiceById = async (req, res) => {
  try {
    ...
    console.error('Get CA legal service by ID error:', error);
    res.status(500).json({
      success: false,
      message: 'Server error',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
