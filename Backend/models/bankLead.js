...
const bankLeadSchema = new mongoose.Schema({
  ...
  hasPanCard: {
    type: String,
    enum: ['yes', 'no'],
    required: [true, 'PAN card availability is required']
  },

  hasGst: {
    type: String,
    enum: ['yes', 'no'],
    required: [true, 'GST availability is required']
  },

  hasUdyamMsmeCertificate: {
    type: String,
    enum: ['yes', 'no'],
    required: [true, 'Udyam/MSME certificate information is required']
  },
  ...
  panNumber: {
    ...
    uppercase: true,
    trim: true,
    match: [/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/, 'PAN number must be valid'],
    required: function () {
      return this.hasPanCard === 'yes';
    }
  },
  ...
  gstNumber: {
    ...
    uppercase: true,
    trim: true,
    match: [/^[0-9A-Z]{15}$/, 'GST number must be 15 characters'],
    required: function () {
  ...
  udyamMsmeNumber: {
    type: String,
    trim: true,
    maxlength: [30, 'Udyam/MSME number cannot exceed 30 characters'],
    required: function () {
      return this.hasUdyamMsmeCertificate === 'yes';
    }
  },
