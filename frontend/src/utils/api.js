...
const apiCall = async (endpoint, options = {}) => {
  ...
  const config = {
    ...
    headers,
    body,
    // Prevent ORB (Opaque Response Blocking) errors
    mode: 'cors', // Explicitly set CORS mode
    credentials: 'include', // Include credentials (cookies, auth headers)
    cache: 'no-cache', // Don't cache to avoid stale responses
  ...
  try {
    ...
    const isExpectedOtpFailure = error.message === 'Invalid or expired OTP';
    if (!isExpectedOtpFailure) {
...
export const loansAPI = {
  ...
  // Submit loan application (public - can be authenticated)
  submitApplication: async (formData, token = null) => {
    // Create FormData for file upload
    const applicationFormData = new FormData();
    
    // Append all form fields
    Object.keys(formData).forEach(key => {
      if (key === 'businessDocuments' && formData[key] instanceof File) {
        applicationFormData.append('businessDocuments', formData[key]);
      } else if (key !== 'businessDocuments') {
        applicationFormData.append(key, formData[key] || '');
      }
    });
    ...
    return apiCall('/loans/applications', {
      method: 'POST',
      headers: headers,
      body: applicationFormData,
      skipJsonHeaders: true, // Don't set JSON headers for FormData
    });
  },
};
...
export const adminLoansAPI = {
  ...
  getApplications: async (token, params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiCall(`/admin/loans/applications${queryString ? `?${queryString}` : ''}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  getApplicationById: async (token, id) => {
    return apiCall(`/admin/loans/applications/${id}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  updateApplicationStatus: async (token, id, status, adminNotes = '') => {
    return apiCall(`/admin/loans/applications/${id}/status`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  // Get loan application stats (admin)
  getApplicationStats: async (token) => {
    return apiCall(`/admin/loans/applications/stats`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
export const mentorAPI = {
  ...
  changePassword: async (token, currentPassword, newPassword) => {
    return apiCall('/mentors/change-password', {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  // Get mentor specializations (public)
  getSpecializations: async () => {
    return apiCall('/mentors/specializations', {
      method: 'GET',
    });
  },
...
export const mentorBookingAPI = {
  ...
  // Create Razorpay order (for web browser - modal)
  ...
  // Create Razorpay payment link (for webview - redirect)
  createPaymentLink: async (token, bookingId) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    return apiCall(`/mentors/bookings/${bookingId}/create-payment-link`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
  ...
  // Verify payment with Razorpay (for cases where handler doesn't execute)
  verifyPayment: async (token, bookingId) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    return apiCall(`/mentors/bookings/${bookingId}/verify-payment`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
  ...
  submitReview: async (token, bookingId, reviewData) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    return apiCall(`/mentors/bookings/${bookingId}/review`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
...
export const trainingAPI = {
  ...
  // Create certificate payment link (for WebView)
  createCertificatePaymentLink: async (token, courseId) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    return apiCall(`/training/certificate/${courseId}/create-payment-link`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
...
export const legalSubmissionAPI = {
  ...
  // Create Razorpay payment link (for WebView)
  createPaymentLink: async (token, submissionId) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    return apiCall(`/legal/submissions/${submissionId}/create-payment-link`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
...
export const caLegalSubmissionAPI = {
  ...
  // Download document (CA)
  downloadDocument: async (token, fileId) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    const url = `${API_BASE_URL}/ca/documents/${fileId}/download`;
...
export const adminNotificationsAPI = {
  ...
  getNotifications: async (token, params = {}) => {
    ...
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    const queryString = new URLSearchParams(params).toString();
    return apiCall(`/admin/notifications${queryString ? `?${queryString}` : ''}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
...
export const pitchAPI = {
  // Submit pitch
  submitPitch: async (token, formData) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    ...
    data.append('oneLinePitch', formData.oneLinePitch);
    ...
    if (formData.financials) {
      data.append('financials', formData.financials);
    }
    
    return apiCall('/pitch/submit', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
  ...
  // Get user's pitches
  getMyPitches: async (token) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    return apiCall('/pitch/my-pitches', {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
  ...
  getPitchById: async (token, pitchId) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    return apiCall(`/pitch/${pitchId}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
  ...
  // Download pitch document
  downloadPitchDocument: async (token, pitchId, documentType) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    const response = await fetch(`${API_BASE_URL}/pitch/${pitchId}/download/${documentType}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
...
// Admin Pitch API
export const adminPitchAPI = {
  // Get all pitches
  getAllPitches: async (token, params = {}) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    const queryParams = new URLSearchParams(params).toString();
    return apiCall(`/admin/pitches${queryParams ? `?${queryParams}` : ''}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
  ...
  getPitchById: async (token, pitchId) => {
    ...
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    return apiCall(`/admin/pitches/${pitchId}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
  ...
  // Update pitch status
  updatePitchStatus: async (token, pitchId, status, rejectionReason, adminNotes) => {
    if (!token || token === 'null' || token === 'undefined') {
      throw new Error('Authentication token is missing. Please login again.');
    }
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    return apiCall(`/admin/pitches/${pitchId}/status`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
  ...
  downloadPitchDocument: async (token, pitchId, documentType) => {
    ...
    const cleanToken = token.trim().replace(/^["']|["']$/g, '');
    const response = await fetch(`${API_BASE_URL}/admin/pitches/${pitchId}/download/${documentType}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${cleanToken}`,
      },
