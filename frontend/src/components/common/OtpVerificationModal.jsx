...
const OtpVerificationModal = ({ isOpen, onClose, phone, purpose, onSuccess }) => {
    ...
    const verifyOtpCode = async (otpCode, triggeredAutomatically = false) => {
        if (isLoading) return;

        const trimmedOtp = (otpCode || '').replace(/\D/g, '');
    ...
    const handleOtpChange = (index, value) => {
        ...
        const sanitizedValue = value ? value.slice(-1) : '';
        const newOtp = [...otp];
        ...
        if (newOtp.every((digit) => digit && digit.length === 1) && !isLoading) {
            verifyOtpCode(newOtp.join(''), true);
        }
    };
