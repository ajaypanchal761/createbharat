...
const ServiceNotification = ({ 
  ...
  serviceName,
  mentorName,
  responseTime, // Dynamic response time from mentor data (e.g., '24 hours', '12 hours', etc.)
  refundAmount,
  onClose 
}) => {
  ...
  const getNotificationContent = () => {
    if (type === 'legal') {
      ...
      const defaultResponseTime = responseTime || '24 hours';
      return {
        icon: '📧',
        title: 'Link Will Be Shared Soon!',
        message: `${mentorName} आपको ${defaultResponseTime} के अंदर Gmail पर session link share करेंगे`,
        guaranteeText: `${defaultResponseTime} Guarantee`,
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white/90 text-sm">
                    {type === 'refund' ? '3-5 Business Days' : (content.guaranteeText || '24 Hours Guarantee')}
                  </p>
                </div>
              </div>
