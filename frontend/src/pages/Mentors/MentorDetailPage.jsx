...
const MentorDetailPage = () => {
      ...
      if (response.success && response.data.mentor) {
        ...
        const reviewCount = typeof apiMentor.reviewCount === 'number'
          ...
          : Array.isArray(apiMentor.reviews)
            ...
            : (typeof apiMentor.totalSessions === 'number' ? apiMentor.totalSessions : 0);
        setMentorData({
  ...
  useEffect(() => {
    ...
    window.addEventListener('mentorRatingUpdated', handler);
    return () => window.removeEventListener('mentorRatingUpdated', handler);
  }, [fetchMentor]);
