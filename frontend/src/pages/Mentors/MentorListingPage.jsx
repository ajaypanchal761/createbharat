...
import { getMentorCategoryById } from '../../data/mentorCategories';
...
const MentorListingPage = () => {
  ...
  const category = getMentorCategoryById(categoryId);
  const categoryDisplayName = category?.name || 'All Mentors';
  const categoryBadge = category?.badge || 'Featured Experts';
  const categoryDescription = category?.description || 'Connect with experienced mentors across industries and skillsets.';
  const headerGradientClass = category ? `bg-gradient-to-r ${category.gradient}` : 'bg-white';
  const headerPrimaryText = category ? 'text-white' : 'text-gray-900';
  const headerSecondaryText = category ? 'text-white/80' : 'text-gray-600';
  const apiCategoryParam = category?.apiCategory || (categoryId && categoryId !== 'all' ? categoryId : '');
  ...
  useEffect(() => {
    const fetchMentors = async () => {
      ...
      try {
        const params = {};
        if (apiCategoryParam) params.category = apiCategoryParam;
    ...
    fetchMentors();
  }, [apiCategoryParam, searchTerm, selectedExperience, selectedRating]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50">
      ...
      <Motion.header
        ...
        className={`relative overflow-hidden shadow-lg sticky top-0 z-50 ${headerGradientClass}`}
      >
        {category && <div className="absolute inset-0 bg-black/10" aria-hidden="true" />}
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="md:hidden flex items-center justify-between h-16">
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${category ? 'bg-white/15 text-white hover:bg-white/25' : 'bg-gray-100 text-gray-900 hover:bg-gray-200'}`}
            >
            ...
            <div className="text-center">
              <h1 className={`text-xs font-semibold tracking-wide ${headerPrimaryText}`}>{categoryDisplayName}</h1>
            </div>
            <button
              onClick={() => navigate('/mentors/login')}
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors ${category ? 'bg-white/20 text-white hover:bg-white/25' : 'bg-orange-600 text-white hover:bg-orange-700'}`}
            >
              Mentor Login
            </button>
          </div>
          ...
          <div className="hidden md:flex items-center justify-between py-4">
            <div className="space-y-2">
              <button
                type="button"
                onClick={() => navigate('/mentors')}
                className={`inline-flex items-center gap-2 text-sm font-medium rounded-full px-4 py-2 transition-colors ${category ? 'bg-white/15 text-white hover:bg-white/25' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
              ...
              <div className="space-y-2">
                <p className={`text-xs uppercase tracking-[0.4em] ${headerSecondaryText}`}>{categoryBadge}</p>
                <h1 className={`text-2xl font-bold ${headerPrimaryText}`}>{categoryDisplayName}</h1>
                <p className={`max-w-2xl text-sm ${headerSecondaryText}`}>{categoryDescription}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <Link
                to="/"
                className={`px-5 py-2 text-sm font-semibold rounded-lg transition-colors ${category ? 'bg-white/20 text-white hover:bg-white/30' : 'bg-white text-orange-600 hover:bg-gray-50'}`}
              >
                Home
              </Link>
              <button
              ...
              <button
                ...
                className={`px-5 py-2 text-sm font-semibold rounded-lg transition-all ${category ? 'bg-white text-slate-900 hover:bg-slate-100' : 'bg-orange-600 text-white hover:bg-orange-700'}`}
              >
                Status
              </button>
            </div>
