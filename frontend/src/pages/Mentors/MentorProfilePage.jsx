...
const MentorProfilePage = () => {
  ...
  const defaultSpecialization = mentorCategories[0] || {
    id: 'business-startup',
  ...
  const [formData, setFormData] = useState({
    ...defaultProfile,
    specialization: defaultProfile.specializationId
  });
  ...
  const deriveSpecializationFromMentor = (mentor) => {
    if (!mentor) {
      return {
        specializationId: defaultProfile.specializationId,
        specializationName: defaultProfile.specializationName
      };
    }

    const specializationId =
      mentor.specializationKey ||
      mentor.specializationId ||
      (mentor.specialization
        ? (mentorCategories.find(cat => cat.name === mentor.specialization)?.id || '')
        : '');

    const specializationCategory = specializationId ? getMentorCategoryById(specializationId) : null;
    const specializationName =
      specializationCategory?.name ||
      mentor.specialization ||
      defaultProfile.specializationName;
  ...
  const mapMentorToProfileState = (mentor) => {
    const {
      specializationId,
      specializationName
    ...
    const profile = {
      ...
      specializationId: specializationId || defaultProfile.specializationId,
      specializationName,
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (response.success && response.data.mentor) {
          ...
          const { profile, form } = mapMentorToProfileState(mentor);
  ...
  const handleChangePasswordSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('mentorToken');
      if (!token) {
        setPasswordError('Please login again to update your password.');
        setIsChangingPassword(false);
        return;
      }
      const cleanToken = token.trim().replace(/^["']|["']$/g, '');
      await mentorAPI.changePassword(cleanToken, currentPassword, newPassword);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const { profile, form } = mapMentorToProfileState(updatedMentor);
        setProfileData(profile);
        setFormData(form);
        setIsEditing(false);
  ...
  const handleCancel = () => {
    setFormData({
      ...profileData,
      specialization: profileData.specializationId
    });
    setIsEditing(false);
    setError('');
  };
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const specializationCategory = getMentorCategoryById(specializationId);
                setFormData(prev => ({
                  ...prev,
        ...
        <form className="space-y-4" onSubmit={handleChangePasswordSubmit}>
          ...
          <div className="flex justify-end">
            <button
              ...
              className="px-5 py-3 bg-orange-600 text-white font-semibold rounded-lg hover:bg-orange-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
