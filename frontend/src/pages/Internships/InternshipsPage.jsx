...
const InternshipsPage = () => {
  ...
  useEffect(() => {
    const loadInternships = async () => {
      try {
        ...
        setApiInternships([]); // Set empty array on error
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const recommendedInternships = allInternships.filter(internship =>
    internship.featured || internship.popular || internship.recommended
  ).slice(0, 8);
  // All internships for "All Internships" section
  const allInternshipsList = allInternships;

  // Update filteredInternships when recommendedInternships changes
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen pb-20 relative overflow-x-hidden bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 md:bg-gradient-to-br md:from-gray-50 md:via-blue-50 md:to-indigo-50">
      ...
      <motion.header
        ...
        className="md:hidden sticky top-0 z-50 bg-white border-b border-gray-200 shadow-md"
      >
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <motion.button
              ...
              className="flex items-center gap-2 px-3 py-2 bg-gray-100 rounded-lg border border-gray-300 hover:bg-gray-200 transition-all duration-200"
            >
            ...
            <Link to="/company/login" title="Company Login">
              <motion.button
                whileHover={{ scale: 1.05, y: -1 }}
                whileTap={{ scale: 0.95 }}
                className="px-3 py-1.5 text-xs font-semibold text-gray-900 border border-gray-300 rounded-lg hover:bg-gray-100 transition-colors"
              >
                Company Login
              </motion.button>
            </Link>
        ...
        {isLoading ? (
          ...
          <div className="hidden md:block max-w-7xl mx-auto">
            <div className="space-y-12">
              {/* Desktop Trending Internships Section - Only show if data exists */}
              ...
              {/* Desktop All Internships Section */}
              {allInternshipsList.length > 0 && (
                <div className="bg-white rounded-2xl p-8 shadow-lg border border-gray-200">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6">
                    {allInternshipsList.slice(0, 12).map((internship) => (
                      <motion.div
                        key={internship.id}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
              ...
              {/* Desktop Recommended Internships Section - Only show if recommended data exists */}
              {recommendedInternships.length > 0 && (
                <div className="bg-white rounded-2xl p-8 shadow-lg border border-gray-200">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6">
                    {recommendedInternships.slice(0, 8).map((internship, index) => (
                      <motion.div
                        key={internship.id}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.5, delay: index * 0.1 }}
                        whileHover={{ scale: 1.02, y: -4 }}
                        className="bg-white border border-gray-200 rounded-xl p-6 shadow-md hover:shadow-lg transition-all duration-300 cursor-pointer"
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex-1">
                            <h3 className="font-bold text-lg mb-1 text-gray-900">{internship.company}</h3>
                            <span className="px-2 py-1 bg-orange-100 text-orange-700 text-xs font-medium rounded-full">
                              Actively hiring
                            </span>
                          </div>
                          <div className={`w-12 h-12 bg-gradient-to-br ${internship.color || 'from-blue-500 to-cyan-500'} rounded-lg flex items-center justify-center text-xl text-white`}>
                            {internship.icon}
                          </div>
                        </div>

                        <h4 className="font-semibold text-lg mb-4 text-gray-900">{internship.title}</h4>
          ...
          <div className="md:hidden">
            {/* Mobile Trending Internships Section - Only show if data exists */}
            {trendingInternships.length > 0 && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    {trendingInternships.map((internship, index) => (
                      <motion.div
                        ...
                        whileTap={{ scale: 0.98 }}
                        className={`flex-shrink-0 w-52 md:w-48 h-48 md:h-48 bg-gradient-to-br ${cardBackgrounds[index % cardBackgrounds.length]} rounded-3xl p-2 border border-white/25 shadow-[0_8px_18px_-16px_rgba(15,23,42,0.55)] hover:shadow-[0_14px_24px_-14px_rgba(59,130,246,0.55)] transition-all duration-500 snap-start cursor-pointer backdrop-blur-sm relative overflow-hidden group`}
                      >
                        {/* Floating Elements */}
                        <motion.div
                          animate={{
                        ...
                        <div className="flex items-start justify-between mb-1">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="px-2 py-1 bg-white/15 text-white text-[11px] font-medium rounded-full border border-white/30 backdrop-blur-sm">
                                Actively hiring
                              </span>
                            </div>
                          </div>
                          <motion.div
                            ...
                            className={`w-6 h-6 bg-gradient-to-br ${internship.color} rounded-2xl flex items-center justify-center text-[11px] shadow-sm flex-shrink-0`}
                          >
                            {internship.icon}
                          </motion.div>
                        </div>
                        ...
                        {/* Job Title */}
                        <h4 className="font-semibold text-white text-[11px] leading-snug mb-1.5">
                          {internship.title}
                        </h4>

                        {/* Details with Icons */}
                        <div className="grid grid-cols-2 gap-x-2 gap-y-1 mb-1.5 text-[10px] text-white/90">
                          ...
                          <div className="flex items-center gap-1">
                            <span className="text-base">💰</span>
                            <span className="truncate">{internship.stipend}{internship.stipendPerMonth || '/month'}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <span className="text-base">🕐</span>
                          ...
                          {internship.category && (
                            <div className="flex items-center gap-1 col-span-2">
                              ...
                              <span className="truncate">{internship.category}</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1">
                            <span className="px-1.5 py-0.5 bg-white/20 text-white/90 text-[11px] rounded-md backdrop-blur-sm">
                              {internship.postedDate}
                            </span>
                          </div>
                          <span className="px-2 py-1 bg-white/15 text-white text-[11px] font-medium rounded-full backdrop-blur-sm">
                            Internship
                          </span>
                        </div>
                        ...
                        <div className="flex gap-2 mt-1">
                          <Link to={`/internships/${internship.id}`}>
                            <motion.div
                              whileHover={{ scale: 1.05 }}
                              whileTap={{ scale: 0.95 }}
                              className="bg-indigo-600 text-white text-center py-1 px-2 rounded-lg font-semibold text-[9px] hover:bg-indigo-700 transition-colors shadow-md"
                            >
                              Apply Now
                            </motion.div>
                          </Link>
                          <Link to={`/internships/${internship.id}`}>
                            <motion.div
                              whileHover={{ scale: 1.05 }}
                              whileTap={{ scale: 0.95 }}
                              className="bg-white border border-indigo-500 text-indigo-600 text-center py-1 px-2 rounded-lg font-semibold text-[9px] hover:bg-indigo-50 transition-colors shadow-sm"
                            >
                              Know More
                            </motion.div>
                          </Link>
                ...
                >
                  {trendingInternships.map((_, index) => (
                    <motion.button
                      ...
                      className={`h-2 rounded-full transition-all duration-300 ${activeCardIndex === index
                        ? 'w-8 bg-indigo-600'
                ...
                <div className="space-y-4">
                    ...
                    >
                      <motion.div
                        ...
                        className="bg-white rounded-2xl p-2 border border-gray-100 shadow-[0_6px_18px_-16px_rgba(79,70,229,0.45)] hover:shadow-[0_10px_22px_-14px_rgba(79,70,229,0.55)] hover:border-indigo-100 transition-all duration-300"
                      >
                        {/* Company Header */}
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="px-2 py-1 bg-orange-50 text-orange-600 text-[11px] font-medium rounded-full border border-orange-200">
                                Actively hiring
                              </span>
                            </div>
                          </div>
                          <motion.div
                            whileHover={{ rotate: 360, scale: 1.1 }}
                            transition={{ duration: 0.6 }}
                            className={`w-5 h-5 bg-gradient-to-br ${internship.color} rounded-2xl flex items-center justify-center text-[11px] shadow-sm flex-shrink-0`}
                          >
                            {internship.icon}
                          </motion.div>
                        </div>
                        ...
                        {/* Job Title */}
                        <h4 className="font-semibold text-gray-800 text-[11px] leading-snug mb-1.5 line-clamp-2">
                          {internship.title}
                        </h4>

                        {/* Details with Icons */}
                        <div className="grid grid-cols-2 gap-x-2 gap-y-1 mb-1.5 text-[10px] text-gray-600">
                          <div className="flex items-center gap-1">
                            <span className="text-base">📍</span>
                            <span className="truncate">{internship.location}</span>
                          </div>
                        ...
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-1">
                            <span className="px-1.5 py-0.5 bg-gray-100 text-gray-500 text-[11px] rounded-md">
                              {internship.postedDate}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          {internship.popular && (
                            <motion.span
                              ...
                              className="px-2 py-1 bg-amber-100 text-amber-700 text-[10px] font-medium rounded-full"
                            >
                              Popular
                            </motion.span>
                          )}
                          {internship.remote && (
                            <span className="px-2 py-1 bg-purple-100 text-purple-700 text-[10px] font-medium rounded-full">
                              Remote
                            </span>
                          )}
                          {internship.urgent && (
                        ...
                        <div className="flex gap-2 mt-1.5">
                          <Link to={`/internships/${internship.id}/apply`} className="flex-1">
                            <motion.button
                              whileHover={{ scale: 1.05 }}
                              whileTap={{ scale: 0.95 }}
                              className="w-full bg-orange-600 text-white text-center py-1.5 px-2.5 rounded-lg font-semibold text-[11px] hover:bg-orange-700 transition-colors shadow-md"
                            >
                              Apply Now
                            </motion.button>
                          </Link>
                          <Link to={`/internships/${internship.id}`} className="flex-1">
                            <motion.button
                              whileHover={{ scale: 1.05 }}
                              whileTap={{ scale: 0.95 }}
                              className="w-full bg-white border border-orange-500 text-orange-600 text-center py-1.5 px-2.5 rounded-lg font-semibold text-[11px] hover:bg-orange-50 transition-colors shadow-sm"
                            >
                              View Details
                            </motion.button>
                          </Link>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {locations.slice(1).map((location) => (
                  <motion.button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${filters.location === location
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {domains.slice(1).map((domain) => (
                  <motion.button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${filters.domain === domain
                      ? 'bg-indigo-500 text-white'
                      : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}
                  >
              ...
              <div className="flex flex-wrap gap-2">
                {workTypes.slice(1).map((type) => (
                  <motion.button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${filters.workType === type
                      ? 'bg-indigo-500 text-white'
                      : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}
                  >
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {stipendRanges.slice(1).map((range) => (
                  <motion.button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${filters.stipendRange === range
                      ? 'bg-indigo-500 text-white'
                      : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}
                  >
              ...
              <div className="flex flex-wrap gap-2">
                {durations.slice(1).map((duration) => (
                  <motion.button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${filters.duration === duration
                      ? 'bg-indigo-500 text-white'
                      : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}
                  >
